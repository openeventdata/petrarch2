...
while len(line) > 0:  # loop through the file
    # note: various Python date utilities can do this more easily
    if '<DATELINE>' in line:
        parse = fin.readline()
        # extract the date, hoping that the format is standardized...
        base = parse.find('(CD')
...
#			ser1 = math.floor(ser/3)+1
        ...
        # check whether file is full; only break files on new stories
        if sentno > SentLimit:
            fout.close()
            fileno += 1
...
# if storyno > 64: break   # debug
    ...
    if '<P>' in line:
        ...
        if seqno > SeqLimit:
            line = fin.readline()
            continue
        # this is the id string for the record
        idstr = prefix + '-' + str(fileno).zfill(2) + '-' + \
            str(storyno).zfill(4) + '-' + str(seqno)
        ...
        while ka < len(parse):
            ...
            if parse[par - 1] != ')':
                ...
                if word[1].isalpha() or word[1].isdigit():
                    ...
                    outstr += ' ['   # convert brackets
                elif '-RRB-' in word:
                    outstr += ']'
                else:
                    outstr += word[1:]  # other punctuation
                ka = par + 1
            else:
                ka += 1
        ...
        outstr = outstr.strip()
        # skip direct quotes
        if nwords < WordLimit or outstr.startswith('`') or outstr.startswith('"') or outstr.startswith("'") or outstr.endswith('`') or outstr.endswith('"') or outstr.endswith("'"):
            if nwords < WordLimit:
                print "Skip: short"
            else:
                print "Skip:", outstr[0]
