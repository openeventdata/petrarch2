...
def open_ErrorFile(sname = '', isunique = ''):
	...
	if len(sname) == 0: # default
		...
		if len(isunique) > 0:
			ErrorfileName = sname[:-3]+ time.strftime("%y%m%d%H%M",time.localtime())+'.txt'
		else: ErrorfileName = sname
...
#	print '--Closing',ErrorfileName," with",ErrorN," errors"
	if ErrorN == 0: os.remove(ErrorfileName)
...
def write_FIN_error(errorstring):
	...
	ErrorFile.write("Location: file " + PETRreader.CurrentFINname + ", line "+str(PETRreader.FINnline)+"\n")
	print errorstring	
